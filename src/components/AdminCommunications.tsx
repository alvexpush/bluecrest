...
import { Bell, CheckCircle2, Mail, RefreshCw, Send, Settings, WalletCards } from 'lucide-react';
import { apiRequest } from '../lib/api';
import AdminSecurity from './AdminSecurity';

export default function AdminCommunications({ users }: { users: any[] }) {
  const [section, setSection] = useState<'email' | 'notifications' | 'withdrawals' | 'security'>('email');
  ...
  const load = useCallback(async () => {
    try {
      const [savedSettings, emailLogs, requests] = await Promise.all([
        apiRequest<any>('/api/v1/admin/email/settings'),
        apiRequest<any[]>('/api/v1/admin/email/logs'),
        apiRequest<any[]>('/api/v1/admin/withdrawals')
      ]);
  ...
  const updateWithdrawal = (id: number, status: string) => run(() => apiRequest(`/api/v1/admin/withdrawals/${id}`, {
    method: 'PATCH', body: JSON.stringify({ status })
  }), `Withdrawal marked ${status.toLowerCase()}.`);

  return <div className="space-y-6">
    <div className="flex flex-wrap gap-2">
      {[{ id: 'email', label: 'Email management', icon: Mail }, { id: 'notifications', label: 'In-app notifications', icon: Bell }, { id: 'withdrawals', label: 'Withdrawals', icon: WalletCards }, { id: 'security', label: 'Security', icon: Settings }].map(item =>
    ...
    {section === 'notifications' && <div className="bg-white rounded-[2rem] p-6 border border-slate-100 max-w-3xl space-y-4">
      ...
      <input className="field-control" placeholder="Optional action link, e.g. /withdrawals" value={notification.action_link} onChange={e => setNotification(value => ({ ...value, action_link: e.target.value }))} />
    ...
    {section === 'withdrawals' && <div className="bg-white rounded-[2rem] p-6 border border-slate-100">
      <h3 className="font-extrabold mb-5">Withdrawal review queue</h3>
      <div className="space-y-3">{withdrawals.map(item => { const user = users.find(user => Number(user.id) === Number(item.user_id)); return <div key={item.id} className="p-4 rounded-2xl border border-slate-100 flex flex-col lg:flex-row lg:items-center gap-4">
        <div className="flex-1"><p className="font-mono text-xs font-bold text-slate-500">{item.reference}</p><p className="text-sm font-bold mt-1">{user ? `${user.first_name} ${user.last_name}` : `User #${item.user_id}`}</p><p className="text-[10px] text-slate-400 uppercase font-bold mt-1">{item.method.replaceAll('_',' ')} · {new Date(item.created_at).toLocaleString()}</p></div>
        <div><p className="font-extrabold">{item.currency} {Number(item.amount).toLocaleString()}</p><p className="text-[9px] text-[#003399] font-bold uppercase">{item.status}</p></div>
        {!['COMPLETED', 'REJECTED', 'FAILED'].includes(item.status) && <div className="flex gap-2">{item.status === 'PENDING' && <button onClick={() => updateWithdrawal(item.id, 'PROCESSING')} className="px-3 py-2 rounded-lg bg-amber-50 text-amber-700 text-[10px] font-bold">Process</button>}<button onClick={() => updateWithdrawal(item.id, 'COMPLETED')} className="px-3 py-2 rounded-lg bg-emerald-50 text-emerald-700 text-[10px] font-bold"><CheckCircle2 className="w-3 h-3 inline mr-1" />Complete</button><button onClick={() => updateWithdrawal(item.id, 'REJECTED')} className="px-3 py-2 rounded-lg bg-rose-50 text-rose-700 text-[10px] font-bold">Reject</button></div>}
      </div>})}</div>
    </div>}
