  ...
  const handleCardAction = async (
    ...
    try {
      const response = await fetch(`/api/v1/cards/${cardId}/${action}`, {
        ...
        body: JSON.stringify(issuanceFee === undefined ? {} : { issuance_fee: issuanceFee })
      });
                ...
                <tbody className="divide-y divide-slate-50">
                  {cards.map(card => (
                    <tr key={card.id}>
                      ...
                      <td className="py-4">
                        <p className="text-xs font-bold text-slate-800">{formatCurrency(Number(card.issuance_fee || 0))}</p>
                        <span className={cn(
                      ...
                      <td className="py-4 text-right">
                          ...
                          <div className="flex justify-end gap-2">
                            <form
                              ...
                              onSubmit={event => {
                                ...
                                const fee = Number((event.currentTarget.elements.namedItem('cardFee') as HTMLInputElement).value);
                                handleCardAction(card.id, 'approve', Number.isFinite(fee) ? fee : 50);
                              }}
                            >
                              <input name="cardFee" type="number" min="0" defaultValue="50" className="w-20 rounded-lg border border-slate-200 bg-slate-50 px-2 py-1 text-xs" />
                              <button className="rounded-lg bg-[#003399] px-3 py-1.5 text-[10px] font-bold text-white">Approve</button>
                            </form>
                        ...
                        {card.status === 'AWAITING_PAYMENT' && (
                          <button onClick={() => handleCardAction(card.id, 'confirm-payment')} className="rounded-xl bg-teal-600 px-3 py-2 text-[10px] font-bold text-white">
